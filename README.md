...
屏内快捷方式默认点击即在电视中启动并自动接入；偏好设置 → 电视应用 → **快捷方式默认打开位置** 可切换为 Windows 桌面，右键快捷方式也可临时指定位置。Edge / Chrome / Brave 的程序及 `.lnk` 快捷方式会请求新窗口，即使浏览器复用进程，也自动识别新窗口并接入，已有窗口不受影响。未能识别的启动器、复用原窗口的其他软件和网址关联程序，可用“接入”手动选择。

屏内“打开”支持程序、`.lnk` 和 `.url`，保留快捷方式启动参数。最多接入 8 个窗口。“切换”轮换应用，“释放”恢复最近接入的窗口，“返回”恢复全部窗口。机身仍可拖动，屏内滚轮传给应用，Ctrl+滚轮缩放电视，Shift+右键打开桌宠菜单。

**电视应用画布分辨率**默认“自动适配”，补偿系统 DPI 缩放，并按电视内容区 800:456 的比例计算尺寸（宽度限制在 1280–1920）。例如 150% 缩放时采用 1536×876，避免浏览器挤在原先 800×456 的小窗口内。也可手动选择 800×456、1280×730、1600×912，即时生效并自动保存。旧版本默认低档位首次升级会迁移到自动适配，此后保留手动选择。

应用画布不继承或修改 Windows 桌面分辨率；电视缩放不会改变画布分辨率。高档位显示更多内容，文字相应缩小。为控制占用，画面仍采样到现有 800×500 电视纹理（含工具栏）。浏览器全屏按实际网页区域补足工具栏和边框占用，不再使用固定高度裁切；网页本身较长时仍保留正常滚动。
