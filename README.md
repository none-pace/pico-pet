...
**电视应用画布分辨率**可选 800×456、1280×720、1600×900，切换即时生效并自动保存，不继承或修改 Windows 桌面分辨率。调整的是应用窗口的独立布局空间；高档位显示更多内容，文字相应缩小。为控制占用，画面仍采样到电视现有的 800×500 屏幕纹理（含工具栏），不是将纹理升级为 1600×900。默认低档位适合小屏阅读；电视缩放不会改变画布分辨率。
