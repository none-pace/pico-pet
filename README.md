...
单应用模式在目标窗口处理移动、恢复和缩放之前约束画布位置，避免浏览器延迟恢复布局时跑到侧边。自动接入会确认窗口已进入容器；隐藏或暂停电视时仍完成接入，画面采样保持暂停。已覆盖连续关闭重开、副屏与菜单再次定位的回归。兼容 DLL 使用临时目录中的版本副本，窗口回调随释放移除；模块保留到目标进程退出，避免容器异常退出后留下失效回调地址。
